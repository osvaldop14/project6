package com.xedb.data;
// Generated 20/01/2016 09:51:24 AM by Hibernate Tools 3.2.4.GA



/**
 * MviewAdvFilterinstance generated by hbm2java
 */
public class MviewAdvFilterinstance  implements java.io.Serializable {


     private MviewAdvFilterinstanceId id;
     private MviewAdvLog mviewAdvLog;

    

    
   
    public MviewAdvFilterinstanceId getId() {
        return this.id;
    }
    
    public void setId(MviewAdvFilterinstanceId id) {
        this.id = id;
    }
    public MviewAdvLog getMviewAdvLog() {
        return this.mviewAdvLog;
    }
    
    public void setMviewAdvLog(MviewAdvLog mviewAdvLog) {
        this.mviewAdvLog = mviewAdvLog;
    }




}


